...
module.exports ={
    uploadFile:(req,res)=>{
        upload(req, res, async (err) => {
            if (err) {
                res.status('400').send({ message: 'Only .xlsx, .xls and .cvs formate is allowed. hii', type: 'danger'});
            } else {
                const params ={
                ...
                s3.upload(params,(err,data) => {
                    ...
                    if(err){
                        ...
                        res.status('201').send(JSON.stringify({ message:'doument save successfully', type: 'success', key: data.Key })); 
